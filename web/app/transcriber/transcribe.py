...
class Transcriber:
    def __init__(self, wav2vec2_model_path="", version=""):
        ...
        (
            ...
            self.vocab,
            self.ctcdecoder,
            self.kenlm_ctcdecoder,
        ) = models.create(self.wav2vec2_model_path, self.version)
    ...
    def split_frames(self, frames, aggressiveness):
        for audio_segment in split(frames, aggressiveness=aggressiveness):
            (
                audio_segment_buffer,
                audio_segment_time_start,
                audio_segment_time_end,
            ) = audio_segment
    ...
    def transcribe(
        self, wav_file_path, max_segment_length=8, max_segment_words=14, withlm=False
    ):
        ...
        for audio_segment in self.split_frames(frames, aggressiveness=1):
            ...
            features = self.processor(
                audio, sampling_rate=16_000, return_tensors="pt", padding=True
            )
            with torch.no_grad():
                ft = features.input_values.to(self.device)
                am = features.attention_mask.to(self.device)
                logits = self.model(ft.float(), attention_mask=am.float()).logits
            transcription, alignment, timesteps = self.ctc_decode(logits, withlm)
            timestep_length = (
                audio_segment_time_end - audio_segment_time_start
            ) / timesteps
            for a in alignment:
            ...
            if len(aligned_words) > 0:
                for transcription, seg_start, seg_end, seg_alignment in segment(
                    aligned_words, max_segment_length, max_segment_words
                ):
    ...
    def transcribe_to_doc(self, wav_file_path, uid):
        ...
        for trans in self.transcribe(wav_file_path):
            ...
            if trans.start_time == 0.0 and index > 1:
                trans.start_time = last_end + 0.1
            transcripts.append(trans)
            index += 1
            last_end = trans.end_time
    ...
    def ctc_decode(self, logits, withlm):
        if withlm:
            (
                ...
                timesteps,
                out_lens,
            ) = self.kenlm_ctcdecoder.decode(logits)
        else:
            beam_results, beam_scores, timesteps, out_lens = self.ctcdecoder.decode(
                logits
            )
        beam_string = "".join(
            self.vocab[n] for n in beam_results[0][0][: out_lens[0][0]]
        )
        alignment = list()
