...
from ..tasks import youtube_dl, convert_file
...
class ExternalViews:
    ...
    @view_config(route_name='home',
                 renderer='../templates/home.html.j2')
    def home(self):
        """
    ...
    def joinregion(self):
        ...
        if 'append' in self.request.params:
            ...
            if 'uid' in self.request.params:
                uid = self.request.params['uid']
                doc = self.request.dbsession.query(
                    Document).filter(Document.uuid == uid).first()
                ignore = True
                if tid < len(doc.transcripts):
                    for trans in doc.transcripts:
                        ...
                        if trans.correction_index == tid:
                            ...
                            trans.candidate = trans.candidate + " " + \
                                merge_to.candidate
                            trans.correction = trans.correction + " " + \
                                merge_to.correction
                    del(doc.transcripts[tid])
        url = self.request.route_url('correct')+"?uid="+uid
        return HTTPFound(location=url)

    @view_config(route_name='correct',
                 renderer='../templates/correct.html.j2')
    def correct(self):
        """
    ...
    def export(self):
        ...
        if 'uid' in self.request.params:
            ...
            candidate = self.request.dbsession.query(Document).filter(
                Document.uuid == uid).first()
...
def transcription_ready(uuid, dbsession):
    if os.path.isfile(uuid + ".wav.failed"):
        ...
        candidate = dbsession.query(Document).filter(
            Document.uuid == uuid).first()
        if candidate:
